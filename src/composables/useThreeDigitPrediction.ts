/**
 * useThreeDigitPrediction
 * Composable สำหรับคำนวณและทำนายเลข 3 ตัวงวดถัดไป
 * ใช้หลักการทางสถิติและการวิเคราะห์รูปแบบ (ปรับจากเลข 2 ตัว)
 */

export const useThreeDigitPrediction = () => {

  /**
   * วิเคราะห์ความถี่ของเลข 3 ตัวที่ออก
   * @param history - Array ของเลข 3 ตัวย้อนหลัง
  ...
  const getColdNumbers = (history: string[], count: number = 5): string[] => {
    ...
    return allNumbers
      ...
      .slice(0, count * 3) // เพิ่มจำนวนเพราะมีตัวเลข 1000 ตัว
      .sort(() => Math.random() - 0.5) // สุ่มเรียง
      .slice(0, count)
  }
  ...
  const analyzePattern = (history: string[]) => {
    ...
    const range = { low: 0, mid: 0, high: 0 } // 000-333, 334-666, 667-999
    ...
    history.forEach(num => {
      ...
      const sum = Math.floor(value / 100) + Math.floor((value % 100) / 10) + (value % 10)
   ...
   * เลขที่ไม่ได้ออกนาน 3-8 งวด มีโอกาสออกสูง (ปรับจาก 3-7 เป็น 3-8 สำหรับ 3 ตัว)
   */
  const analyzeGap = (history: string[]): Record<string, number> => {
    ...
    // สุ่มเลข 200 ตัวจากทั้งหมด 1000 ตัวเพื่อประหยัดการคำนวณ
    const sampledNumbers = []
    for (let i = 0; i < 200; i++) {
      sampledNumbers.push(Math.floor(Math.random() * 1000).toString().padStart(3, '0'))
    }
  ...
  /**
   * ทำนายเลข 3 ตัวงวดถัดไป
   * @param history - Array ของเลข 3 ตัวย้อนหลัง
   * @param predictionCount - จำนวนเลขที่ต้องการทำนาย (default: 12)
   * @returns Object ที่มีเลขทำนายและข้อมูลวิเคราะห์
   */
  const predictNextNumbers = (history: string[], predictionCount: number = 12) => {
    if (history.length < 3) {
      return {
        predictions: [],
        confidence: 0,
    ...
    // 1. Gap Numbers - เลขที่ไม่ได้ออกนาน 3-8 งวด (30%)
  ...
  const generatePatternBasedNumbers = (recent: string[], count: number): string[] => {
    ...
    const firstDigit = Math.floor(lastValue / 100)
    const secondDigit = Math.floor((lastValue % 100) / 10)
    const thirdDigit = lastValue % 10

    // สร้างเลขใกล้เคียงกับงวดล่าสุด
    const variations = [
      // เปลี่ยนตัวท้าย
      `${firstDigit}${secondDigit}${(thirdDigit + 1) % 10}`.padStart(3, '0'),
      `${firstDigit}${secondDigit}${(thirdDigit + 9) % 10}`.padStart(3, '0'),
      // เปลี่ยนตัวกลาง
      `${firstDigit}${(secondDigit + 1) % 10}${thirdDigit}`.padStart(3, '0'),
      `${firstDigit}${(secondDigit + 9) % 10}${thirdDigit}`.padStart(3, '0'),
      // เปลี่ยนตัวหน้า
      `${(firstDigit + 1) % 10}${secondDigit}${thirdDigit}`.padStart(3, '0'),
      `${(firstDigit + 9) % 10}${secondDigit}${thirdDigit}`.padStart(3, '0'),
      // สลับตัวเลข
      `${thirdDigit}${secondDigit}${firstDigit}`.padStart(3, '0'),
      `${secondDigit}${firstDigit}${thirdDigit}`.padStart(3, '0'),
    ]
  ...
  const generateMixedNumbers = (history: string[], count: number): string[] => {
    ...
    for (let i = 0; i < digitArray.length && results.length < count; i++) {
      for (let j = 0; j < digitArray.length && results.length < count; j++) {
        for (let k = 0; k < digitArray.length && results.length < count; k++) {
          const newNum = digitArray[i] + digitArray[j] + digitArray[k]
  ...
  const calculateConfidence = (history: string[], predictions: string[]): number => {
    ...
    const diversityScore = predictions.length >= 12 ? 30 : (predictions.length / 12) * 30
