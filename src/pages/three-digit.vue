...
import { useThreeDigitPrediction } from '../composables/useThreeDigitPrediction'
...
const getStorageKey = (lotteryId: string) => `three_digit_history_${lotteryId}`
...
const addHistory = () => {
  ...
  if (!/^\d{3}$/.test(value)) {
    alert('ใส่เลข 3 ตัวเท่านั้น (000-999)')
    return
  }
...
const calculatePrediction = () => {
  ...
  // ทำนายเลขงวดถัดไป (เลข 3 ตัวแนะนำ 12 เลข)
  predictions.value = predictNextNumbers(history.value, 12)
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div class="bg-white/90 backdrop-blur rounded-2xl shadow-lg p-6">
          ...
          <div class="mt-6 p-4 bg-blue-50 rounded-xl text-sm text-gray-600">
            ...
            <ul class="space-y-1 text-xs">
              <li>1. กรอกเลข 3 ตัวของงวดย้อนหลัง (000-999)</li>
        ...
        <div class="space-y-4">
          ...
          <div v-if="predictions" class="bg-white/90 backdrop-blur rounded-2xl shadow-lg p-6">
            ...
            <div v-if="predictions.predictions.length > 0" class="grid grid-cols-3 gap-3 mb-4">
              <div
